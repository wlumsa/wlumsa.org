...
const mediaDoc = await getMedia("Mosque");
const services= await fetchIIAServices();
const questions = await fetchFAQ();


const IIA: React.FC<HeroProps> = ({ mediaDocs=mediaDoc}) => {
  ...
  return (
      ...
      <div className="hero-content text-center relative z-10">
        <div className="">
          ...
          <div className="my-2 flex flex-row items-center justify-center gap-4 ">
            <button className="btn  border-none text-white bg-primary hover:bg-[#35995A]">Learn More</button>
    ...
    <div className=" px-20 items-center  ">
      ...
      {questions.map((question, index) => ( 
        ...
        <div key={index}
          className="collapse-title text-lg font-semibold bg-secondary text-primary-content peer-checked:bg-secondary peer-checked:text-secondary-content">
           {question.Question}
        </div>
        <div key={index}
