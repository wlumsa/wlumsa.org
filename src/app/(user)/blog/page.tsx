...
import { fetchBlogPostsByQuery, fetchBlogPostsByQueryAndCategory, getCategories } from "@/Utils/datafetcher";
import { Category as PostCategory, Exec, Media, Category as PayloadCategory, Post } from "@/payload-types";
      ...
      <section className="border-b border-base-300 bg-base-100">
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
          <div className="border-y border-base-300 py-6 sm:py-8">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <h1 className="max-w-3xl text-balance font-heading text-5xl font-bold leading-[0.98] text-primary sm:text-6xl lg:text-7xl">
                  MSA Blog
                </h1>
              </div>
              <p className="max-w-xl text-base leading-relaxed text-base-content/75 sm:text-lg lg:pb-2">
                Articles for spiritual growth, campus life, and academic steadiness from the Wilfrid Laurier Muslim Students&apos; Association.
              </p>
            </div>
          </div>
          ...
          <div className="pt-8 lg:pt-10">
            {featuredPost ? (
              <FeaturedPost post={featuredPost} eyebrow={hasFilters ? "Top result" : "Latest article"} />
            ) : (
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {remainingPosts.length > 0 ? (
          <>
            <div className="mb-5 mt-10 flex items-end justify-between gap-4">
              <div>
                ...
                <h2 className="mt-1 font-heading text-2xl font-bold text-base-content sm:text-3xl">
                  More from the blog
                </h2>
              </div>
            </div>
          ...
          <div className="mt-10 flex min-h-[320px] flex-col items-center justify-center border border-dashed border-base-300 bg-base-200/50 px-6 py-12 text-center">
            ...
            <h2 className="mt-4 font-heading text-2xl font-bold text-base-content">
              No articles found
            </h2>
...
function FeaturedPost({ post, eyebrow }: { post: Post; eyebrow: string }) {
  const image = post.header_image?.find((item): item is Media => typeof item === "object" && item !== null);
  const category = typeof post.categories === "object" ? post.categories as PostCategory : null;
  const author = post.authors?.find((item): item is Exec => typeof item === "object" && item !== null);
  const href = getPostHref(post);
  const date = post.publishedAt || post.createdAt ? new Date(post.publishedAt || post.createdAt) : new Date();
  const formattedDate = Number.isNaN(date.getTime()) ? "Date unavailable" : format(date, "MMMM dd, yyyy");
        ...
        <div className="relative min-h-[220px] overflow-hidden bg-base-300 md:min-h-full">
          {image?.url ? (
            ...
            <div className="flex h-full min-h-[220px] items-center justify-center bg-base-200">
              <span className="font-heading text-6xl font-bold text-primary/15">MSA</span>
            </div>
          )}
        </div>
        <div className="flex flex-col justify-between p-5 sm:p-7">
          <div>
            <div className="mb-4 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] font-semibold uppercase tracking-wide text-base-content/55">
              ...
              {category?.title && (
                <>
                  <span aria-hidden className="text-base-content/25">/</span>
                  <span className="text-primary">{category.title}</span>
                </>
              )}
            </div>
            <h2 className="text-balance font-heading text-2xl font-bold leading-tight text-base-content transition-colors group-hover:text-primary sm:text-3xl">
              {post.title}
            </h2>
            {post.description && (
