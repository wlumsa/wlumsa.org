...
export async function POST(req: Request) {
  try {
    ...
    if (distributionListId === 1) {
      const { data: id, error: errorBroadcastCreate } =
    ...
    // Else send to specific distribution list using normal api
      ...
      const distributionList: (number | Individual)[] | null | undefined =
        await getDistributionList(distributionListId);
      ...
      const validDistributionList: Individual[] = distributionList.filter(
        (item): item is Individual => {
