...
const schema = z.object({
  ...
  email: z
    .string()
    .email("Invalid email")
    .endsWith("@mylaurier.ca", "Must be a mylaurier email"),
  studentId: z
    .string()
    .regex(/^\d+$/, "Student ID must be a number")
    .length(9, "Student ID must be 9 digits"),
...
export async function memberSignup(formData: FormData) {
  ...
  if (!validatedFields.success) {
    return {
      ...
      message: `Failed to sign up. ${Object.values(
        validatedFields.error.flatten().fieldErrors
      ).join(", ")}`,
    };
  }

  const { firstName, lastName, email, studentId, newsLetter } =
