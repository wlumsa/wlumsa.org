...
interface TimingsData {
  timings: Timings;
  date: string; // This will be a formatted date string
}
...
const fetchTimings = async (): Promise<TimingsData[]> => {
  ...
  const q = query(adhanCollectionRef, orderBy("date", "asc"));
  const querySnapshot = await getDocs(q);
  
  return querySnapshot.docs.map(doc => {
    // Format the Firebase timestamp to a date string
    const timestamp = doc.data().date;
    const date = timestamp.toDate().toDateString();
    return {
      timings: doc.data() as Timings,
      date // Now this is a human-readable date string
    };
  });
};
  ...
  return (
    ...
    <div className="container mx-auto p-8">
      ...
      <div className="overflow-x-auto">
        ...
        <p>
          Based on Waterloo Masjid timings and Hanafi Asr calculation
        </p>
      </div>
      <Footer/>
    </div>
