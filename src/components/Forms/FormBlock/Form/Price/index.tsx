...
> = ({
  ...
  const totalPrice = React.useMemo(() => {
    const sortedConditions = [...priceConditions].sort(
      (a, b) => a.itemCount - b.itemCount
    );
    ...
    <div className="mx-auto  min-h-[18rem] ">
      <Width width={width || 100}>
        <div className="py-4  ">
          ...
          <label
            htmlFor={name}
            className="label min-w-96 text-3xl font-semibold text-gray-600"
          >
            {label}:
          </label>
          <h1 className="text-xl">
            ({items.length}) {items.length === 1 ? "paymentNum" : "Players"} -{" "}
          ...
          <div className="min-h-[24px]">{errors[name] && <Error />}</div>
        </div>
      </Width>
    </div>
  );
