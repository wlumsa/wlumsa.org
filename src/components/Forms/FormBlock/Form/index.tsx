...
export const FormBlock: React.FC<FormBlockType & { id?: string }> = (props) => {
  ...
  const steps = formFromProps.fields.map(
    (field: FormFieldBlock, index: number) => {
      const Field: React.FC<any> | undefined =
        fields[field.blockType as keyof typeof fields];
      return (
        <div className="flex w-full flex-grow" key={index}>
          <AnimatePresence>
            <motion.div
              ...
              animate="visible"
              exit="exit"
              className="flex h-[18rem-full] w-full flex-col justify-between rounded-xl px-8"
            >
              {Field ? (
                <Field
                  form={formFromProps}
      ...
      try {
        ...
        if (newsletterField && "name" in newsletterField) {
          ...
          const isNewsletterChecked =
            Array.isArray(newsletterValue) && newsletterValue.length > 0;

          if (isNewsletterChecked) {
            // Extract name and email from the form data
            const nameField = formFromProps.fields.find(
              (field) =>
                field.blockType === "text" &&
                "name" in field &&
                (field.name === "name" || field.name === "firstName")
            );
            const emailField = formFromProps.fields.find(
              (field) =>
        ...
        try {
          ...
          if (req.status >= 400) {
            throw new Error(
              res.errors?.[0]?.message || "Failed to create submission"
            );
          }
        ...
        if (
          paymentFieldName &&
          data[paymentFieldName] &&
          Number(data[paymentFieldName]) > 0
        ) {
          try {
            ...
            if (!session?.url) {
              throw new Error(
                "No session URL returned from createCheckoutSession"
              );
            }
  ...
  return (
    <div className="mt-8 flex min-h-[80vh] w-full flex-col items-center justify-center bg-gradient-to-b from-white via-slate-50 to-white px-4 py-10 dark:from-base-100 dark:via-base-200 dark:to-base-100">
      <div className="w-full max-w-5xl">
        <h1 className="text-2xl font-semibold tracking-tight text-slate-900 md:text-3xl dark:text-base-content">
          {formFromProps.title}
        </h1>
        ...
        <div className="mt-3 h-1.5 w-full rounded-full bg-slate-100 dark:bg-base-300">
          <div
            ...
            style={{
              width: `${
                totalSteps
                  ? (Math.min(currStepIndex + 1, totalSteps) / totalSteps) * 100
                  : 0
              }%`,
            }}
          />
      ...
      <div className="flex w-full items-center justify-center">
        {formFromProps.submissionLimit === 0 ? (
        ...
        ) : formFromProps.releaseDate &&
          new Date() < new Date(formFromProps.releaseDate) ? (
          <div className="p-4 text-center">
            <h2 className="text-xl font-bold text-base-content">
              This form will be releasing at{" "}
              {new Date(formFromProps.releaseDate).toLocaleString("en-US", {
                timeZone: "America/New_York",
              })}
            </h2>
          </div>
        ) : formFromProps.closeDate &&
          new Date() > new Date(formFromProps.closeDate) ? (
          <div className="p-4 text-center">
            <h2 className="text-xl font-bold text-base-content">
              This was closed at{" "}
              {new Date(formFromProps.closeDate).toLocaleString("en-US", {
                timeZone: "America/New_York",
              })}
            </h2>
          </div>
          ...
          <>
            ...
            {error && (
              <div className="text-error">{`${error.status || "500"}: ${
                error.message || ""
              }`}</div>
            )}
            {!hasSubmitted && (
              <div className="w-full min-w-0 max-w-5xl">
                <FormProvider {...formMethods}>
                  <form
                    className="flex h-[64svh] w-full flex-col rounded-3xl border border-slate-200 bg-white/80 shadow-2xl shadow-slate-200/40 backdrop-blur md:h-[480px] dark:border-base-300 dark:bg-base-200/90 dark:shadow-base-300/20"
                    id={formID}
                  ...
                  >
                    {currStepIndex === steps.length ? (
                      ...
                      <div className="h-fit flex-grow overflow-y-auto p-2 md:p-8">
                        {step}
                      </div>
                    )}

                    <div className="flex items-center justify-between border-t border-slate-100 px-6 md:px-8  dark:border-base-300">
                      <button
                        type="button"
                      ...
                      {currStepIndex === steps.length - 1 ? (
                        ...
                        >
                          ...
                          {isLoading && (
                            <span className="loading loading-spinner items-center justify-center"></span>
                          )}
                        </button>
                      ) : (
                        <button
                          ...
                          className="btn btn-secondary px-10 text-lg"
                          onClick={handleNext}
                          disabled={currStepIndex === steps.length}
                        >
                          {currStepIndex === steps.length
                            ? submitButtonLabel || "Submit"
                            : "Next"}
