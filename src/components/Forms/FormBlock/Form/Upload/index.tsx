  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      if (!presignRes.ok) {
        ...
        throw new Error(
          err.error ?? "Could not prepare upload. Please try again."
        );
      }
    ...
    <div className="mx-auto min-h-[12rem]">
      <div className="max-w-3xl">
        ...
        {uploadedFileName && !uploading ? (
          <div className="mt-4 flex items-center gap-3 rounded-xl border border-green-200 bg-green-50 px-4 py-3 dark:border-success/30 dark:bg-success/10">
            <svg
              className="h-5 w-5 shrink-0 text-green-800 dark:text-success"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
            ...
            <span className="flex-1 truncate text-sm text-green-800 dark:text-success">
              {uploadedFileName}
            </span>
            <button
              type="button"
          ...
          <label
            className={`mt-4 flex w-full cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed transition-colors
              ${
                uploading
                  ? "cursor-not-allowed border-slate-200 bg-slate-50 dark:border-base-300 dark:bg-base-300/10"
                  : "border-slate-300 bg-slate-50 hover:border-primary hover:bg-primary/5 dark:border-base-300 dark:bg-base-300/10"
              }`}
          ...
          >
            <div className="flex flex-col items-center justify-center px-4 py-8">
              {uploading ? (
                ...
                <>
                  <svg
                    className="mb-2 h-8 w-8 text-slate-400 dark:text-base-content/40"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
        ...
        {uploadError && (
          <p className="mt-2 text-sm text-red-600 dark:text-error">
            {uploadError}
          </p>
        )}
