...
function mapWeeklyRowToDay(row: Record<string, unknown>): DayTiming {
  return {
    ...
    fajr_iqamah:
      firstStringValue(row, WEEKLY_TIME_KEYS.fajr_iqamah) || undefined,
    dhuhr: firstStringValue(row, WEEKLY_TIME_KEYS.dhuhr_start) || undefined,
    dhuhr_iqamah_1:
      firstStringValue(row, WEEKLY_TIME_KEYS.dhuhr_iqamah_1) || undefined,
    dhuhr_iqamah_2:
      firstStringValue(row, WEEKLY_TIME_KEYS.dhuhr_iqamah_2) || undefined,
    asr: firstStringValue(row, WEEKLY_TIME_KEYS.asr_start) || undefined,
    asr_iqamah_1:
      firstStringValue(row, WEEKLY_TIME_KEYS.asr_iqamah) || undefined,
    maghrib: firstStringValue(row, WEEKLY_TIME_KEYS.maghrib_start) || undefined,
    maghrib_iqamah:
      firstStringValue(row, WEEKLY_TIME_KEYS.maghrib_iqamah) || undefined,
    isha: firstStringValue(row, WEEKLY_TIME_KEYS.isha_start) || undefined,
    isha_iqamah:
      firstStringValue(row, WEEKLY_TIME_KEYS.isha_iqamah) || undefined,
  };
}

function getWeeklyUpcomingDays(
  weeklyTimetable: WeeklyPrayerTimetable | null
): DayTiming[] {
...
const PrayerTimesTable: React.FC<PrayerTimesTableProps> = ({
  ...
  const monthlyUpcomingDays = (currentMonthTimings?.slice(
    today.getDate() - 1,
    today.getDate() + 6
  ) || []) as DayTiming[];
  const upcomingDays =
    weeklyUpcomingDays.length > 0 ? weeklyUpcomingDays : monthlyUpcomingDays;
  ...
  const getFormattedIqamah = (day: DayTiming, key: TimingKey) => {
    if (key === "dhuhr") {
      return `${
        day["dhuhr_iqamah_1"]
          ? `${day["dhuhr_iqamah_1"]} ${timingDictonary["dhuhr_iqamah_1"]}`
          : ""
      }${day["dhuhr_iqamah_1"] && day["dhuhr_iqamah_2"] ? " | " : ""}${
        day["dhuhr_iqamah_2"]
          ? `${day["dhuhr_iqamah_2"]} ${timingDictonary["dhuhr_iqamah_2"]}`
          : ""
      }`;
    }

    if (key === "asr") {
      return day["asr_iqamah_1"]
        ? `${day["asr_iqamah_1"]} ${timingDictonary["asr_iqamah_1"]}`
        : "N/A";
    }

    const iqamahKey = `${key}_iqamah` as TimingKey;
    return day[iqamahKey]
      ? `${day[iqamahKey]} ${timingDictonary[iqamahKey]}`
      : "N/A";
  };
          ...
          return (
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3">
                {timesToShow.map((key) => {
                  const formattedTiming = day[key]
                    ? `${day[key]} ${timingDictonary[key]}`
                    : "--";
                  ...
                  return (
                    <div
                      key={`${dayIndex}-${key}`}
                      className="rounded-xl border border-base-300 bg-base-200/50 px-3 py-2 text-center"
                    >
                      ...
                      {key === "dhuhr" &&
                        ...
                        jummahTimes.map((jummah, index) => (
                          <p
                            key={
                              jummah.id ??
                              `${jummah.building}-${jummah.timing}-${index}`
                            }
                            className="mt-1 text-xs font-medium text-info"
                          >
                            {formatJummahTiming(jummah.timing)} (
                            {jummah.building})
                          </p>
                        ))}
                    </div>
            ...
            {timesToShow.map((key, keyIndex) => (
              <tr key={keyIndex}>
                ...
                {upcomingDays.map((day, dayIndex) => {
                  ...
                  return (
                    <td
                      key={`${dayIndex}-${keyIndex}`}
                      className="whitespace-nowrap px-6 py-4 text-sm text-base-content/80"
                    >
                      ...
                      {key === "dhuhr" &&
                        ...
                        jummahTimes.map((jummah, index) => (
                          ...
                          >
                            {formatJummahTiming(jummah.timing)} at (
                            {jummah.building})
                          </div>
                        ))}
                    </td>
