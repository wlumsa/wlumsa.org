...
const MasjidCard: React.FC<MasjidCardProps> = ({
  name,
  image,
  address,
  websiteLink,
  ...
  return (
    <div
      className={
        "dark:border-base-700 rounded-xl border border-base-300 bg-base-100 p-4 transition-all duration-200 hover:border-primary/30 hover:shadow-xl sm:p-6 dark:bg-base-200 dark:hover:shadow-2xl"
      }
    >
      <div className="flex flex-row gap-6">
        {image && image.length > 0 && image[0] ? (
          <Image
            ...
            decoding="async"
            loading="lazy"
            className="rounded-lg object-cover transition-transform duration-200 hover:scale-105 motion-reduce:hover:scale-100"
          />
        ) : (
        ...
        <div className="flex min-w-0 flex-1 flex-col justify-between">
          ...
          <div className="mt-3 flex gap-3 sm:mt-4 sm:gap-4">
            {googleMapsLink && googleMapsLink.length > 0 && (
              <Link
                href={`${googleMapsLink}`}
                target="_blank"
