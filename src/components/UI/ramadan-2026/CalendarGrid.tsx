...
export function CalendarGrid({
  ...
  const handleArrowNavigation = (
    ...
    const currentIndex = flatDays.findIndex(
      (day) => day.isoDate === currentISO
    );
    if (currentIndex < 0) return;

    let nextIndex = currentIndex;
    if (event.key === "ArrowRight")
      nextIndex = Math.min(currentIndex + 1, flatDays.length - 1);
    if (event.key === "ArrowLeft") nextIndex = Math.max(currentIndex - 1, 0);
    if (event.key === "ArrowDown")
      nextIndex = Math.min(currentIndex + 7, flatDays.length - 1);
    ...
    <section className="space-y-4 rounded-2xl border border-base-300 bg-base-100/95 p-4 shadow-md shadow-base-content/10 md:p-5 lg:pb-11">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center justify-end gap-1.5">
          {COMPACT_LEGEND.map((item) => (
            <span
              key={item.label}
              className={`font-body rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-base-content/85 md:text-xs ${item.className}`}
            >
              {item.label}
            </span>
          ))}
      ...
      {grouped.map(([monthKey, monthDays]) => {
        const firstDate = parseISODate(
          monthDays[0]?.isoDate || `${monthKey}-01`
        );
        const leadingBlanks = new Array(firstDate.getDay()).fill(null);

        return (
          <div key={monthKey} className="space-y-2">
            <h3 className="font-heading text-center text-base font-bold text-base-content/90">
              {monthLabel(monthKey)}
            </h3>

            <div className="font-body grid grid-cols-7 gap-1 text-center text-[11px] uppercase tracking-wide text-base-content/60">
              {WEEKDAYS.map((day) => (
                <div key={day}>
            ...
            <div className="grid grid-cols-7 gap-1 md:gap-2">
              {leadingBlanks.map((_, index) => (
                <div
                  ...
                  className="min-h-[72px] rounded-xl bg-base-200/40 md:min-h-[92px]"
                />
              ))}

              {monthDays.map((day) => {
                const isSelected = selectedISO === day.isoDate;
                const isEvenNightInLastTen =
                  day.isLastTen && !day.isOddNight && !day.isEid;
                ...
                return (
                  <button
                    ...
                    type="button"
                    aria-label={`${formatShortDate(day.isoDate)} ${
                      day.fastIndex
                        ? `${getOrdinal(day.fastIndex)} fast`
                        : "Eid al-Fitr"
                    }`}
                    ...
                    onKeyDown={(event) =>
                      handleArrowNavigation(event, day.isoDate)
                    }
                  >
                    <p className="font-body whitespace-nowrap text-[10px] text-neutral/80 md:text-[11px]">
                      {formatShortDate(day.isoDate)}
                    </p>

                    <p className="font-body mt-1 whitespace-nowrap text-[10px] font-semibold text-base-content md:text-[12px]">
                      {day.fastIndex ? (
                        <>
                          <span className="md:hidden">{`${day.fastIndex}F`}</span>
                          <span className="hidden md:inline">{`${getOrdinal(
                            day.fastIndex
                          )} Fast`}</span>
                        </>
                      ) : (
                    ...
                    <div className="mt-1 flex flex-wrap justify-center gap-1">
                      {day.isFirstTaraweeh ? (
                        <span className="font-body hidden rounded-md bg-info px-1.5 py-0.5 text-[10px] font-medium text-info-content md:inline-flex">
                          First Taraweeh
                        </span>
                      ) : null}
                      {day.isLastTen ? (
                        <span className="font-body hidden rounded-md bg-accent px-1.5 py-0.5 text-[10px] font-medium text-accent-content md:inline-flex">
                          Last 10
                        </span>
                      ) : null}
                      {day.isOddNight ? (
                        <span className="font-body hidden rounded-md border border-warning/60 bg-warning px-1.5 py-0.5 text-[10px] font-semibold text-warning-content md:inline-flex">
                          Odd Night
                        </span>
                      ) : null}
                      {day.isEid ? (
                        <span className="font-body hidden rounded-md bg-secondary px-1.5 py-0.5 text-[10px] font-medium text-secondary-content md:inline-flex">
                          Eid
                        </span>
                      ) : null}
