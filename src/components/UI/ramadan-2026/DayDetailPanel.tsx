import type { RamadanDay } from "@/lib/ramadan2026";
import {
  EID_AL_FITR_2026_PRAYER_LOCATION,
  EID_AL_FITR_2026_PRAYER_TIME,
  getLastThirdOfNight,
  getOrdinal,
} from "@/lib/ramadan2026";

type DayDetailPanelProps = {
  day?: RamadanDay;
  nextDay?: RamadanDay;
  isMobileOpen: boolean;
  onCloseMobile: () => void;
};

function PrayerTimeRow({
  label,
  value,
  emphasize = false,
}: {
  label: string;
  value?: string;
  emphasize?: boolean;
}) {
  return (
    <div
      className={`relative flex items-center justify-between rounded-xl border px-3 py-2.5 ${
        emphasize
          ? "bg-secondary/12 border-secondary/45 shadow-sm"
          : "border-base-300 bg-base-200/50"
      }`}
    >
      <span
        className={`font-body text-sm ${
          emphasize ? "text-base-content" : "text-neutral/80"
        }`}
      >
        {label}
      </span>
      <span className="font-body text-sm font-bold tabular-nums text-base-content">
        {value || "--:--"}
      </span>
    </div>
  );
}

function PanelBody({
  day,
  nextDay,
}: Omit<DayDetailPanelProps, "onCloseMobile" | "isMobileOpen">) {
  if (!day) {
    return (
      <p className="font-body text-sm text-base-content/70">
        Select a day to view details.
      </p>
    );
  }

  const prayerTimes = day.prayerTimes;
  const lastThird = getLastThirdOfNight(prayerTimes, nextDay?.prayerTimes);
  return (
    <div className="space-y-4">
      <div>
        <h3 className="font-heading text-lg font-bold text-primary">
          {day.dayLabel}
        </h3>
        <p className="font-body text-sm text-base-content/75">
          {day.fastIndex ? `${getOrdinal(day.fastIndex)} Fast` : "Eid al-Fitr"}
        </p>
      </div>

      <div className="space-y-2 rounded-2xl border border-base-300 bg-base-100 p-3 shadow-sm">
        <div className="flex items-center justify-between">
          <p className="font-body text-[11px] font-semibold uppercase tracking-wide text-base-content/60">
            Full Prayer Schedule
          </p>
          <p className="font-body text-[10px] text-base-content/55">
            Waterloo local time
          </p>
        </div>
        <PrayerTimeRow
          label="Fajr (Suhoor ends)"
          value={prayerTimes?.fajr}
          emphasize
        />
        <PrayerTimeRow label="Sunrise" value={prayerTimes?.sunrise} />
        <PrayerTimeRow label="Dhuhr" value={prayerTimes?.dhuhr} />
        <PrayerTimeRow label="Asr" value={prayerTimes?.asr} />
        <PrayerTimeRow
          label="Maghrib (Iftar)"
          value={prayerTimes?.maghrib}
          emphasize
        />
        <PrayerTimeRow label="Isha" value={prayerTimes?.isha} />
      </div>

      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-1">
        <div className="rounded-lg border border-base-300 bg-base-200/60 px-3 py-2">
          <p className="font-body text-[11px] uppercase tracking-wide text-neutral/80">
            Night Status
          </p>
          <p className="font-body text-sm font-semibold text-base-content">
            {day.isEid
              ? "Eid"
              : day.isOddNight
              ? "Odd night in the last 10"
              : day.isLastTen
              ? "Last 10 nights"
              : "Regular Ramadan night"}
          </p>
        </div>
        <div className="rounded-lg border border-base-300 bg-base-200/60 px-3 py-2">
          <p className="font-body text-[11px] uppercase tracking-wide text-neutral/80">
            Last Third Starts
          </p>
          <p className="font-heading text-base font-bold tabular-nums text-base-content">
            {lastThird || "--:--"}
          </p>
          <p className="font-body text-[11px] text-neutral/80">
            Based on Maghrib to next day&apos;s Fajr.
          </p>
        </div>
      </div>

      {day.isEid ? (
        <div className="rounded-xl border border-secondary/35 bg-secondary/10 p-3 shadow-sm">
          <p className="font-body text-[11px] font-semibold uppercase tracking-wide text-base-content/70">
            Eid Prayer
          </p>
          <p className="font-body mt-1 text-sm font-semibold text-base-content">
            {EID_AL_FITR_2026_PRAYER_LOCATION}
          </p>
          <p className="font-body text-sm text-base-content/80">
            {EID_AL_FITR_2026_PRAYER_TIME}
          </p>
          <ul className="font-body mt-2 list-disc space-y-1 pl-4 text-sm text-base-content/80">
            <li>Bring your own prayer mats.</li>
            <li>Carpool to ease parking and traffic.</li>
          </ul>
        </div>
      ) : null}
    </div>
  );
}

export function DayDetailPanel({
  day,
  nextDay,
  isMobileOpen,
  onCloseMobile,
}: DayDetailPanelProps) {
  return (
    <>
      <aside className="hidden rounded-2xl border border-base-300 bg-base-100/95 p-4 shadow-md shadow-base-content/10 lg:sticky lg:top-24 lg:block">
        <PanelBody day={day} nextDay={nextDay} />
      </aside>

      {day && isMobileOpen ? (
        <div className="fixed inset-0 z-40 lg:hidden">
          <button
            type="button"
            className="absolute inset-0 bg-black/40"
            aria-label="Close day details"
            onClick={onCloseMobile}
          />
          <div className="absolute inset-x-0 bottom-0 rounded-t-2xl border border-base-300 bg-base-100 p-4 shadow-2xl">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="font-heading text-base font-bold text-primary">
                Day Details
              </h3>
              <button
                type="button"
                className="btn btn-ghost btn-sm"
                onClick={onCloseMobile}
              >
                Close
              </button>
            </div>
            <div className="max-h-[62vh] overflow-y-auto pb-4">
              <PanelBody day={day} nextDay={nextDay} />
            </div>
          </div>
        </div>
      ) : null}
    </>
  );
}
