...
const CountingNumber = ({
  ...
  useEffect(() => {
    springValue.on("change", (latest) => {
      if (ref.current) {
        ref.current.textContent = `${prefix}${Number(
          latest.toFixed(0)
        )}${suffix}`;
      }
    });
  ...
  return (
    <section id="stats">
      <div className="container mx-auto max-w-5xl px-4 py-8">
        ...
        <div
          className="stats stats-vertical grid grid-cols-1 gap-8 shadow lg:stats-horizontal md:grid-cols-2 lg:grid-cols-3"
          ref={ref}
        >
          {stats.map((stat, index) => (
            <motion.div
              ...
              custom={index}
              className="stat flex flex-col items-center"
            >
              <h3 className="mb-2 text-center text-4xl font-bold md:text-5xl lg:text-6xl">
                <CountingNumber
                  value={stat.value}
                  prefix={stat.prefix}
                  suffix={stat.suffix}
